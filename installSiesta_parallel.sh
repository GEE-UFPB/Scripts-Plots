...
sh ../Src/obj_setup.sh
cd ../Src
./configure --enable-mpi FC=/usr/bin/mpif90 --with-blacs=/usr/lib/libblacs-openmpi.so --with-scalapack=/usr/lib/libscalapack-openmpi.so
sudo rm arch.make
sudo wget https://github.com/GEE-UFPB/Scripts-Plots/raw/master/arch.make
cp arch.make ../Obj
cd ../Obj
make
#########################
